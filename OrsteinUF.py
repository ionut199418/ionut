...
class OrsteinUF(Kern):
    ...
    def K(self, X, X2=None):
        ...
        #if self.timescale_H != self.timescale_F:
        ...
        #else: 
         #   print("Warning, the timescales need to be different.")
...
# Two different timescales, i.e tau_H and tau_F.
    ...
    def Kdiag(self, X):
        ...
        #ret[:] = np.clip(self.sigma_a * self.timescale_H * self.timescale_F * 0.5,1e-3,np.inf)
        #ret[:] = np.abs(self.sigma_a * self.timescale_H * self.timescale_F * 0.5 + 1e-3)
        return ret
