...
class OrsteinUFnew2(Kern):
    
    def __init__(self,input_dim,timescale_H=2.,timescale_F=1.,sigma_H=1.,active_dims=None):
        super(OrsteinUFnew2, self).__init__(input_dim, active_dims, 'OUF')
        assert input_dim == 1 #, "For this kernel we assume input_dim=1"
        self.timescale_H = Param('timescale_H', timescale_H,Logexp())
        self.timescale_F = Param('timescale_F', timescale_F,Logexp())
        self.sigma_H = Param('sigma_H', sigma_H,Logexp())
        self.link_parameters(self.timescale_H,self.timescale_F,self.sigma_H)
...
# Two different timescales, i.e tau_H and tau_F.
    ...
    def update_gradients_full(self, dL_dK, X, X2):
        ...
        dsigma = Ke
        dtimescaleH = self.sigma_H * 1/(self.timescale_H- self.timescale_F)**2 * (np.exp(-r_H)*(1+r_H)*(self.timescale_H- self.timescale_F)-f)
        dtimescaleF =  self.sigma_H * 1/(self.timescale_H- self.timescale_F)**2 * (-np.exp(-r_F)*(1+r_F)*(self.timescale_H- self.timescale_F)+f)
        self.sigma_H.gradient = np.sum(dsigma*dL_dK)
